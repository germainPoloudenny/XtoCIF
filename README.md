...
### Multi-GPU Training
deCIFer's training loop mirrors the single-GPU pipeline even when launched with multiple devices. Simply keep the same configuration file used for single-GPU experiments and start the run with `torchrun`:

```bash
torchrun --standalone --nproc_per_node=4 bin/train.py --config configs/paper.yaml
```

- `batch_size` remains the per-device micro batch, exactly as in the single-GPU training loop, so you can reuse your main-branch hyperparameters.
- To keep the optimizer trajectory identical, the trainer divides `gradient_accumulation_steps` by the world size internally (so pick a value that is divisible by the number of GPUs you plan to use).

With this setup, multi-GPU runs behave identically to the reference single-GPU training while finishing faster.
...
- `--beam-size <int>` / `--length-penalty <float>`: Enable beam search decoding during evaluation; keep at `1` to match the main branch’s ancestral sampling.
- `--beam-deterministic`: Use deterministic beam expansions instead of sampling when beam search is enabled.
...
#### Optional Arguments:
- `--top-k <int>`: Keep only the best `k` generations per sample when merging, ranked by the chosen metric.
- `--top-k-metric {rwp,l2}`: Metric used for the per-sample ranking; `rwp` mimics the paper’s filtering.
