...
def space_group_to_crystal_system(space_group):
    """Map an international space-group number to a crystal system index."""
    try:
        if 1 <= space_group <= 2:
            return 1  # Triclinic
...
def discrete_to_continuous_xrd(
    ...
    **kwargs, # Ignoring
    ...
    if intensity_scale_range is not None:
        intensity_scale = torch.empty(batch_size, 1, device=device, dtype=dtype).uniform_(*intensity_scale_range)  # Shape: (batch_size, 1)
        batch_iq = batch_iq * intensity_scale

    # Convert FWHM to standard deviations
    sqrt_two_ln_two = math.sqrt(2.0 * math.log(2.0))
    sigma_gauss = fwhm / (2 * sqrt_two_ln_two)
    gamma_lorentz = fwhm / 2

    # Expand q grid once for broadcasting
    q_cont_expanded = q_cont.view(1, num_q_points, 1)            # Shape: (1, num_q_points, 1)
    max_num_peaks = batch_q.shape[1]
    iq_cont = torch.zeros(batch_size, num_q_points, device=device, dtype=dtype)
    valid_peaks = (batch_q != 0).to(dtype)

    # Heuristic chunk size so intermediate tensors stay within ~256MB per batch
    target_chunk_bytes = 256 * 1024 * 1024
    intermediate_multiplier = 4  # delta_q, gaussian, lorentzian, pseudo
    element_size = torch.tensor([], dtype=dtype, device='cpu').element_size()
    denom = batch_size * num_q_points * element_size * intermediate_multiplier
    if denom > 0:
        chunk_size = max(1, min(max_num_peaks, target_chunk_bytes // denom))
    else:
        chunk_size = max_num_peaks or 1

    # Process peaks in memory-friendly chunks
    for start in range(0, max_num_peaks, chunk_size):
        end = min(start + chunk_size, max_num_peaks)
        chunk_mask = valid_peaks[:, start:end]
        if torch.count_nonzero(chunk_mask).item() == 0:
            continue
        q_chunk = batch_q[:, start:end]
        iq_chunk = batch_iq[:, start:end]
        chunk_mask = chunk_mask.unsqueeze(1)

        delta_q = q_cont_expanded - q_chunk.unsqueeze(1)
    ...
    if noise_range is not None:
        noise_scale = torch.empty(batch_size, 1, device=device, dtype=dtype).uniform_(*noise_range)  # Shape: (batch_size, 1)
        noise = torch.randn(batch_size, num_q_points, device=device, dtype=dtype) * noise_scale
        iq_cont += noise

    # Apply random masking
    if mask_prob is not None:
        mask = (torch.rand(batch_size, num_q_points, device=device, dtype=dtype) > mask_prob).to(dtype)
        iq_cont *= mask
...
def get_atomic_props_block(composition, oxi=False):
    """Build the CIF loop that stores electronegativity/radii metadata."""
    noble_vdw_radii = {
        "He": 1.40,
        "Ne": 1.54,
        "Ar": 1.88,
...
def replace_symmetry_loop_with_P1(cif_str):
    """Force space group loop to P1 so downstream tokenisation is stable."""
...
def reinstate_symmetry_loop(cif_str, space_group_symbol):
    """Recreate the symmetry loop using the predicted space-group symbol."""
...
def remove_cif_header(cif_str):
    """Strip the ``data_`` section header."""
    lines = cif_str.split('\n')
    cif_lines = []
    for line in lines:
        line = line.strip()
...
def remove_oxidation_loop(cif_str):
    """Remove oxidation loop entries because they are often noisy."""
...
def format_occupancies(cif_str, decimal_places=4):
    """Normalize the formatting of fractional occupancies."""
...
def extract_numeric_property(cif_str, prop, numeric_type=float):
    """Helper for pulling numeric ``_cell_`` attributes out of the CIF."""
...
def extract_data_formula(cif_str):
    """Return the ``data_`` formula string without further parsing."""
...
def extract_formula_nonreduced(cif_str):
    """Return the non-reduced chemical formula."""
...
def replace_data_formula_with_nonreduced_formula(cif_str):
    """Ensure the ``data_`` section contains the full (non-reduced) formula."""
...
def round_numbers(cif_str, decimal_places=4):
    """Round floating-point literals to make tokenisation deterministic."""
...
def get_unit_cell_volume(a, b, c, alpha_deg, beta_deg, gamma_deg):
    """Compute the cell volume from lengths and angles."""
...
def extract_volume(cif_str):
    """Extract (or compute) the unit-cell volume."""
...
def bond_length_reasonableness_score(cif_str, tolerance=0.32, h_factor=2.5):
    """Score whether the CIF contains implausible bond lengths."""
    """
...
def is_space_group_consistent(cif_str):
    """Check symmetry consistency between declared and parsed space groups."""
    structure = Structure.from_str(cif_str, fmt="cif")
    try:
...
def is_formula_consistent(cif_str):
    """Ensure parsed formula matches the data_ header."""
    try:
        parser = CifParser.from_string(cif_str)
    except:
        parser = CifParser.from_str(cif_str)
...
def is_atom_site_multiplicity_consistent(cif_str):
    """Verify that site multiplicities do not exceed the symmetry maximum."""
...
def is_sensible(cif_str, length_lo=0.5, length_hi=1000., angle_lo=10., angle_hi=170.):
    """Sanity-check unit-cell lengths/angles."""
...
def is_valid(cif_str, bond_length_acceptability_cutoff=1.0):
    """Aggregate structural validity checks into a single boolean."""
...
def evaluate_syntax_validity(cif_str, bond_length_acceptability_cutoff=1.0):
    """Return a dict summarising all validity heuristics for a CIF."""
    validity_dict = {
        "formula": False,
...
def space_group_symbol_to_number(symbol):
    """Lightweight parser for Hermann–Mauguin symbols."""
...
def get_metrics(ckpt_path):
    """Load evaluation metrics from a checkpoint folder."""
...
def get_rmsd(cif_string_sample, cif_string_gen, matcher, supercell_matcher=None):
    """
    Compute the RMSD between two CIF structures using the provided matcher(s).

    Returns a tuple ``(rmsd_value, matcher_mode, failure_cause)`` where
    ``matcher_mode`` is ``\"primitive\"`` or ``\"supercell\"`` depending on the
    matcher that succeeded, and ``failure_cause`` is a short label describing
    why the RMSD could not be computed.
    """

    def _load_structure(cif_string):
        return Structure.from_str(cif_string, fmt="cif")

    def _normalize_rmsd(raw_value):
    ...
    def _infer_failure_cause(structure_sample, structure_gen):
        if structure_sample is None or structure_gen is None:
            return "invalid_structure"
        try:
            if len(structure_sample) != len(structure_gen):
                return "atom_count_mismatch"
            if not structure_sample.composition.almost_equals(structure_gen.composition):
    ...
    try:
        structure_sample = _load_structure(cif_string_sample)
        structure_gen = _load_structure(cif_string_gen)
    ...
    matchers = [
        ("primitive", matcher),
        ("supercell", supercell_matcher),
    ]

    for mode, active_matcher in matchers:
        if active_matcher is None:
            continue
        try:
            rmsd_raw = active_matcher.get_rms_dist(structure_sample, structure_gen)
        except Exception as exc:
            print(exc)
            continue
        rmsd_value = _normalize_rmsd(rmsd_raw)
        if rmsd_value is not None:
            return rmsd_value, mode, None

    failure_cause = _infer_failure_cause(structure_sample, structure_gen)
...
def plot_loss_curves(paths, ylog=True, xlog=False, xmin=None, xmax=None, ymin=None, ymax=None, offset=0.02, plot_metrics=True, figsize=(10, 5)):
    """Visualise training/validation losses for one or more runs."""
    # Apply Seaborn style
