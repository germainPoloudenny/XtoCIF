...
@dataclass
class DeciferConfig:
    """Configuration for the autoregressive decoder and PXRD conditioning."""
    block_size: int = 1024
    vocab_size: int = 372
    n_layer: int = 8
    n_head: int = 8
...
class LayerNorm(nn.Module):
    """LayerNorm with configurable bias to match nanoGPT defaults."""
...
class CausalSelfAttention(nn.Module):
    """Multi-head self-attention block with optional boundary-aware masking."""
...
class MLP(nn.Module):
    """Standard GPT-style feed-forward block."""
...
class Block(nn.Module):
    """Transformer block that combines attention + MLP with residuals."""
...
class Decifer(nn.Module):
    """Autoregressive backbone used by deCIFer."""
