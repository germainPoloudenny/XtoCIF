...
def write_publication_quality_plots(
    ...
    *,
    ...
    for metric in metric_names:
        ...
        if "beam_size" in frame.columns:
            beam_sizes = np.sort(frame["beam_size"].dropna().to_numpy(dtype=float))
            if beam_sizes.size and np.all(np.isclose(beam_sizes, np.round(beam_sizes))):
                integer_ticks = np.unique(np.round(beam_sizes).astype(int))
                ax.set_xticks(integer_ticks)
                ax.set_xticklabels([str(tick) for tick in integer_ticks])

        if layout.prefer_integer_ticks:
            ax.yaxis.set_major_locator(matplotlib.ticker.MaxNLocator(integer=True))

        # Add a clear baseline (B=1) horizontal line when available
        try:
            top1_subset = frame[(frame.get("variant") == "top1")]
            if not top1_subset.empty and "beam_size" in top1_subset and metric in top1_subset:
                b1_rows = top1_subset[top1_subset["beam_size"].astype(float) == 1.0]
                if not b1_rows.empty:
                    b1_val = float(pd.to_numeric(b1_rows.iloc[0][metric], errors="coerce"))
                    if np.isfinite(b1_val):
                        ax.axhline(
                            y=b1_val,
                            color="#000000",
                            linestyle="--",
        ...
        slug = _slugify_metric(metric)
        # Save both PNG (for quick previews) and PDF (vector for publication)
        fig.savefig(output_dir / f"{slug}.png", bbox_inches="tight", dpi=300)
        fig.savefig(output_dir / f"{slug}.pdf", bbox_inches="tight")
        plt.close(fig)
