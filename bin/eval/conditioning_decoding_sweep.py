...
from bin.eval._metrics_utils import (
    compute_structure_match_breakdown,
    compute_validity_breakdown,
)
...
def _compute_validity_rate(frame: pd.DataFrame) -> Dict[str, object]:
    return compute_validity_breakdown(frame)
...
def _collect_metrics(
    ...
    metrics.update(compute_structure_match_breakdown(frame))
...
def _append_rwp_rankings(frame: pd.DataFrame) -> pd.DataFrame:
    """Add a ranking column derived from Rwp values when available."""

    if "rwp" not in frame.columns:
        return frame

    numeric = pd.to_numeric(frame["rwp"], errors="coerce")
    ...
    if ranking.notna().any():
        frame["rwp_ranking"] = ranking.astype("Int64")
    return frame
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Top-k parameter used by the sampling decoding variant (omit to disable top-k filtering).",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Override the length penalty forwarded to evaluate.py (omit to use evaluate.py's default).",
    )
    parser.add_argument(
        "--nproc-per-node",
        type=int,
...
def main() -> None:
    ...
    if args.sampling_top_k is not None and args.sampling_top_k < 1:
        ...
        for decoding in decoding_variants:
            ...
            if args.skip_existing and pickle_path.exists():
                ...
                if args.length_penalty is not None:
                    evaluate_cmd.extend(["--length-penalty", str(args.length_penalty)])
    ...
    frame = _append_rwp_rankings(frame)
