...
def worker(input_queue, eval_files_dir, done_queue):
    ...
    supercell_matcher = StructureMatcher(
        ...
        # ``StructureMatcher`` only accepts ``num_sites``, ``num_atoms`` or
        # ``volume`` for ``supercell_size`` in the installed pymatgen release.
        # The previous ``\"minimize\"`` value triggered a runtime error before any
