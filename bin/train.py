...
def unwrap_model(model: torch.nn.Module) -> torch.nn.Module:
    """Unwrap DistributedDataParallel wrappers for checkpointing/logging."""
...
class TrainConfig:
    ...
    enhanced_pxrd_conditioning: bool = True  # toggle for the new PXRD conditioning pipeline
...
def setup_datasets(C, distributed=False, show_progress=True, *, rank: int = 0, world_size: int = 1):
    ...
    base_seed = getattr(C, "seed", None)
    sampler_seed = int(base_seed) if base_seed is not None else 0
...
if __name__ == "__main__":
    ...
    resume_from_best = bool(getattr(C, "resume_from_best", False))
    enhanced_pxrd_conditioning = bool(getattr(C, "enhanced_pxrd_conditioning", True))

    # Setup distributed environment (no-op when single process)
    use_distributed, world_size, rank, local_rank = setup_distributed(C)

    # When multiple GPUs participate, divide the accumulation steps evenly.
    if world_size > 1:
        original_accum = int(getattr(C, "gradient_accumulation_steps", 1))
        if original_accum % world_size != 0:
            raise ValueError(
                f"gradient_accumulation_steps={original_accum} must be divisible by world_size={world_size} "
                "to keep the multi-GPU batch identical to the single-GPU run."
            )
        adjusted_accum = original_accum // world_size
        if adjusted_accum < 1:
            raise ValueError(
                f"gradient_accumulation_steps/world_size must be >= 1 (got {adjusted_accum}). "
                "Increase gradient_accumulation_steps or reduce world_size."
            )
        if rank == 0:
            print(
                f"[INFO] Using gradient_accumulation_steps={adjusted_accum} per GPU (from {original_accum}) "
                "to match the single-GPU effective batch.",
                flush=True,
            )
        C.gradient_accumulation_steps = adjusted_accum
    ...
    def get_batch(split, *, show_progress=False):
        ...
        use_enhanced_conditioning = enhanced_pxrd_conditioning

        # Initialize lists to store packed sequences and start indices
        start_indices_list = []
        cond_list = []

        def _extend_conditional_entries(cond_entries, *, move_to_target: bool):
            if isinstance(cond_entries, torch.Tensor):
                tensor_entries = _move_to_target(cond_entries) if move_to_target else cond_entries
                cond_list.extend(tensor_entries)
            else:
                for entry in cond_entries:
                    if move_to_target and isinstance(entry, torch.Tensor):
                        entry = _move_to_target(entry)
                    cond_list.append(entry)

        def _collect_conditionals(batch):
            if not C.condition:
                return
            if use_enhanced_conditioning:
                xrd_q = batch['xrd.q']
                xrd_iq = batch['xrd.iq']
                ...
                _extend_conditional_entries(cond_entries, move_to_target=True)
            else:
                cond_entries = discrete_to_continuous_xrd(batch['xrd.q'], batch['xrd.iq'], **augmentation_kwargs)['iq']
                _extend_conditional_entries(cond_entries, move_to_target=False)
        ...
        while True:
            ...
            # Final fallback: skip this partial batch to stay aligned with single-GPU ordering
            if is_main_process:
                print(
                    f"[WARN] {split}: insufficient tokens to form block_size={C.block_size} after {max_extra_fetches} extra fetches. "
                    "Skipping this batch to avoid duplicating sequences.",
                    flush=True,
                )
            ...
            return get_batch(split, show_progress=show_progress)
    ...
    while not stop_training:
        ...
        # Forward/backward update with gradient accumulation and optional scaling
